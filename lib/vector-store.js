...
// ============================================================
// 7. 向量化 chunks（用于生产持久化向量记录）
// ============================================================

/**
 * 为一批 chunks 生成持久化向量记录格式（用于 kl.createVectorsBatch）。
 *
 * 与 buildIndex 的区别：
 *   - buildIndex：全库 chunks 现算，用于检索时的索引构建
 *   - embedChunks：单个 std 的 chunks 离线计算，生成持久化 vector 记录
 *
 * 本函数只负责"现算向量"这一步，结果写入 vectors 表由调用方负责（通过 kl.createVectorsBatch）。
 *
 * M6 更新：向量记录带 version_id 字段（需求第 36 节 - RAG 兼容）
 *
 * @param {Array<{id, content, ...}>} chunks
 * @param {Object} [opts] { model, indexName, versionId }
 * @returns {Array<{chunkId, model, dim, vec, indexName, versionId}>}
 */
function embedChunks(chunks, { model = 'tfidf-v1', indexName = 'main', versionId = null } = {}) {
  ...
  return chunks.map((c) => ({
    ...
    vec: vectorize(c.content || '', vocab, idfMap),
    indexName,
    // M6: 带上 versionId 用于 RAG 检索时的版本过滤
    versionId: versionId || c.versionId || null,
  }));
}
