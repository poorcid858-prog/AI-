...
/**
 * 创建新能力。自动生成 ID（cap_<type>_001 等），初始版本为 1。
...
/** 能力审核状态常量 */
const REVIEW_STATUS = {
  NONE: null,               // 无审核流程（直接发布）
  PENDING_REVIEW: 'pending_review',  // 待审核
  APPROVED: 'approved',     // 审核通过（等待发布）
  REJECTED: 'rejected',     // 审核驳回
};

/**
 * 提交审核：能力从"草稿"状态提交审核。要求该能力有草稿。
 * @param {string} capId
 * @param {string} submittedBy
 * @returns {Object} 更新后的能力
 */
function submitForReview(capId, submittedBy) {
  const caps = listCapabilities();
  const idx = (Array.isArray(caps) ? caps : []).findIndex((c) => c && c.id === capId);
  if (idx === -1) throw fail("能力不存在", 404);
  const cap = caps[idx];
  if (!cap.draft) throw fail("没有草稿可提交审核", 400);
...
/**
 * 获取待审核能力列表（含草稿内容摘要，供审核人查看）。
 * @returns {Object[]}
 */
function getPendingReviewCapabilities() {
  return listCapabilities().filter((c) =>
    c && c.review && c.review.status === REVIEW_STATUS.PENDING_REVIEW
  );
}

/**
 * 审核决定：通过或驳回。通过后 review.status = approved，等待"发布"。
 * @param {string} capId
 ...
 * @returns {Object} 更新后的能力
 */
function reviewCapability(capId, decision, reviewer, note) {
  if (!['approved', 'rejected'].includes(decision)) {
    throw fail(`无效的审核决定: ${decision}`, 400);
  }
  const caps = listCapabilities();
  const idx = (Array.isArray(caps) ? caps : []).findIndex((c) => c && c.id === capId);
  if (idx === -1) throw fail("能力不存在", 404);
  const cap = caps[idx];
  if (!cap.review || cap.review.status !== REVIEW_STATUS.PENDING_REVIEW) {
    throw fail("该能力当前无待审核请求", 400);
  }

  cap.review.status = decision;
  cap.review.reviewedAt = new Date().toISOString();
  cap.review.reviewedBy = reviewer;
  cap.review.note = note || '';
  cap.updatedAt = new Date().toISOString();

  // 审核通过后，将草稿内容替换为一份"待发布"快照，便于后续 publish
  if (decision === 'approved' && cap.draft) {
    cap.review.approvedContent = JSON.parse(JSON.stringify(cap.draft.content));
    cap.review.approvedVersion = cap.draft.version;
  }
  saveCapabilities(caps);
  ...
  appendAuditLog({
    ...
    capId,
    reviewedBy: reviewer,
    detail: decision === 'approved' ? '审核通过' : `审核驳回: ${note || ''}`,
  });
  return cap;
}

/**
 * 发布能力 —— 审核通过后可将草稿发布为生效版（复用 publishDraft，但检查审核要求）。
 * 若能力存在 review 且 status=approved，则自动走审核发布。
 * @param {string} capId
 * @param {string} publishedBy
 * @returns {Object} 更新后的能力
 */
function publishWithReview(capId, publishedBy) {
  const cap = getCapability(capId);
  if (!cap) throw fail("能力不存在", 404);
  // 如果有审核流程且未通过，不允许直接发布
  if (cap.review && cap.review.status === REVIEW_STATUS.PENDING_REVIEW) {
    throw fail("能力待审核中，审核通过后才能发布", 400);
  }
  return publishDraft(capId, publishedBy);
}
