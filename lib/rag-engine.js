...
function loadApprovedIndex() {
  // 决策 6：只入 PUBLISHED，不收 NEED_REVIEW。listRetrievableVectors 包含两者，
  // 这里再过滤一次以保持"approved"严格语义。
  const vectors = kl.listRetrievableVectors().filter(
    (v) => v.status === kl.STD_STATUS.PUBLISHED
  );
