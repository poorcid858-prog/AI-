...
/**
 * 发布文档（审核通过 → 生成向量 → 转 published）
 *
 * 与 review() 分离的原因：review() 测试已锁定 approved 后 isCurrent 仍为 false，
 * 这是有意设计（审核通过 ≠ 上线）。发布是后续独立动作。
 *
 * 流程：
 *   1. 权限：auth.canReview（与审核同一批角色）
 *   2. raw 必须存在，取最新 std（listStdByRaw[0]）
 *   3. std.status 必须是 APPROVED，否则 409
 *   4. 取该 std 的全部 chunks；为空则 409（防御性，正常流程不会发生）
 *   5. vectorStore.embedChunks(chunks) → kl.createVectorsBatch(...)
 *   6. kl.markChunksEmbedded(chunkIds)
 *   7. kl.publishStd(std.id) —— 转 published，级联同步 chunks/vectors.status，
 *      旧版本自动归档，raw.currentStdId 更新
 *   8. 返回 getDocumentView(rawId)
 *
 * @param {Object} user 操作者（需有 review 权限）
 * @param {string} docId raw 文档 id
 * @returns {Object} 发布后的 view
 */
function publishDocument(user, docId) {
  // 1. 权限
  if (!auth.canReview(user)) {
    throw fail('当前角色无发布权限（仅审核专员可发布）', 403);
  }
  ...
  // 3. 取最新 std（同 getDocumentView 逻辑）
  const std = kl.listStdByRaw(docId)[0] || null;
  if (!std) throw fail('文档尚无可发布的版本', 409);
  if (std.status !== kl.STD_STATUS.APPROVED) {
    throw fail(`文档当前状态为 ${std.status}，仅已审核通过(approved)的版本可发布`, 409);
  }

  // 4. 取全部 chunks
  const chunks = kl.listChunksByStd(std.id);
  if (chunks.length === 0) {
    throw fail('文档无切片数据，无法发布', 409);
  }

  // 5. 向量化
  const vs = require('./vector-store');
  const vectorRecords = vs.embedChunks(chunks, { model: 'tfidf-v1', indexName: 'main' });
  if (vectorRecords.length === 0) {
    throw fail('向量化失败', 500);
  }

  // 6. 写入 vectors 表（保持旧形式：{chunkId, model, dim, vec, indexName}）
  kl.createVectorsBatch(
    vectorRecords.map((v) => ({
      chunkId: v.chunkId,
      model: v.model,
      dim: v.dim,
  ...
  // 7. 标记 chunks 向量化完成
  kl.markChunksEmbedded(chunks.map((c) => c.id));

  // 8. 发布 std（级联同步 chunks / vectors.status，旧版本自动归档）
