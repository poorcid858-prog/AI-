...
function read(name, fallback = []) {
  ...
  if (fs.existsSync(fp)) {
    try {
      ...
      // 去除 BOM（EF BB BF），避免 JSON.parse 因 BOM 失败
      value = JSON.parse(raw.replace(/^﻿/, ''));
    } catch (err) {
      console.error(
        `[store] ${name}.json 读取/解析失败，已使用默认值回退。` +
        `文件=${fp} 错误=${err.name}: ${err.message}`
      );
      // 若备份文件存在，尝试从 .bak 恢复（防御：主文件损坏但备份完好）
      const bak = `${fp}.bak`;
      if (fs.existsSync(bak)) {
        try {
          ...
          console.warn(`[store] 已从备份 ${bak} 恢复 ${name}.json`);
        } catch (bakErr) {
          console.error(`[store] 备份 ${bak} 也读取失败，使用默认值: ${bakErr.message}`);
          value = fallback;
        }
      } else {
        console.warn(`[store] 无备份文件（${bak}），使用默认值 ${JSON.stringify(fallback).slice(0, 50)}...`);
        value = fallback;
      }
    }
  } else {
    // 文件不存在：返回默认值（不落盘，避免在迁移等场景造成副作用）。
    value = fallback;
  }
