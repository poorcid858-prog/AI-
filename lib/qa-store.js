...
// ============================================================
// 优秀案例库（双向反馈 回路一：优秀问答→案例库）
// ============================================================

const EXAMPLES_FILE = "qa-examples";

/** qa-examples.json 空形态 */
function emptyExamples() {
  return { version: 1, updatedAt: new Date().toISOString(), examples: [] };
}

/** 读 qa-examples（文件不存在/损坏 → 空形态） */
function readExamples() {
  const v = store.read(EXAMPLES_FILE, emptyExamples());
  if (!v || typeof v !== "object" || !Array.isArray(v.examples)) return emptyExamples();
  return v;
}

/**
 * 追加一条优秀问答到案例库 / qa-examples.json（双向反馈 回路一）。
 * @param {object} input { role, question, answer, tags?, source?, userId? }
 * @returns {object} 新增的案例
 */
function addExample(input) {
  const it = input || {};
  if (!it.role) throw new Error('addExample 需要 role');
  if (!it.question || typeof it.question !== 'string') throw new Error('addExample 需要 question（非空字符串）');
  if (!it.answer || typeof it.answer !== 'string') throw new Error('addExample 需要 answer（非空字符串）');

  const doc = readExamples();
  // 去重：同 role + 同 question（归一化后）不重复入库
  const normQ = normalize(it.question);
  const dup = doc.examples.some(
    (e) => e.role === it.role && normalize(e.question || '') === normQ
  );
  if (dup) {
    return { ok: false, duplicated: true, id: null, reason: '同 role 同问题已存在' };
  }
  ...
  store.write(EXAMPLES_FILE, doc);
  return { ok: true, duplicated: false, id: example.id, example };
}

/** 按角色过滤案例库 */
function listExamples(role) {
  const doc = readExamples();
  const list = role
    ? doc.examples.filter((e) => e.role === role)
    : doc.examples;
  return list.slice().sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
}

/** 从 qa-history 按 sessionId+turn 找到一轮问答（user+ai），用于"标记为优秀案例" */
function getQAPair(sessionId, turn) {
  const records = listBySession(sessionId).filter((r) => r.turn === turn);
  const user = records.find((r) => r.type === 'user');
  const ai = records.find((r) => r.type === 'ai');
